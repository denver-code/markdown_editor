...
class NewlineFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    if (newValue.text.length > oldValue.text.length &&
      ...
      final previousLine =
          text.line(text.locationAt(newValue.selection.start - 1).line - 1);
      final length = previousLine.characters.takeWhile(isSpace).length;
      final indent = ''.padLeft(length);
      var header = bulletListHeader(previousLine, length) ??
          numberListHeader(previousLine, length);
  ...
  String? numberListHeader(Line previousLine, int indent) {
    final string = String.fromCharCodes(previousLine.characters
        .skip(indent)
        .takeWhile((char) => char != periodCharcode));
...
String lineTrimmed(Line line, int indent) =>
    String.fromCharCodes(line.characters.skip(indent)).trimRight();
